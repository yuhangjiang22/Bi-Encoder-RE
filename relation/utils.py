...
def generate_relation_data(entity_data, use_gold=False, context_window=0):
    ...
    ace_entity_types = ['ART', 'ORG-AFF', 'GEN-AFF', 'PHYS', 'PER-SOC', 'PART-WHOLE']
        ...
        for i, sent in enumerate(doc):
            ...
            for x in range(len(sent_ner)):
                for y in range(len(sent_ner)):
                    ...
                    elif type_pair in biored_entity_pairs: # This task is only for BioRED
                    ...
                    # else:
                    elif sub.label in scierc_entity_types and obj.label in scierc_entity_types:
                        ...
                        # captured.append([sub.span, obj.span])
                        # captured.append([obj.span, sub.span])
                    elif sub.label in ace_entity_types and obj.label in ace_entity_types:
                        label = gold_rel.get((sub.span, obj.span), 'no_relation')
                        sample = {}
                        sample['docid'] = doc._doc_key
                        sample['id'] = '%s@%d::(%d,%d)-(%d,%d)' % (
